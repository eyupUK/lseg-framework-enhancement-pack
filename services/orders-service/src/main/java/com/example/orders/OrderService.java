...
public final class OrderService {
    ...
    public OrderDecision placeOrder(OrderRequest request) {
        ...
        processedRequests.compute(request.correlationId(), (key, existing) -> {
            ...
            boolean reserved = inventoryGateway.reserve(request.sku(), request.quantity(), request.correlationId());
            BigDecimal total = request.unitPrice().multiply(BigDecimal.valueOf(request.quantity()))
                    .setScale(2, RoundingMode.HALF_UP);
            OrderDecision decision = reserved
                    ? new OrderDecision(request.orderId(), OrderDecision.Status.ACCEPTED, total, null)
                    : new OrderDecision(request.orderId(), OrderDecision.Status.REJECTED, total, "OUT_OF_STOCK");
            result.set(decision);
    ...
    private static void validate(OrderRequest request) {
        ...
        if (request.unitPrice() == null || request.unitPrice().compareTo(MIN_PRICE) < 0
                || request.unitPrice().compareTo(MAX_PRICE) > 0) {
            throw new IllegalArgumentException("unitPrice must be between 0.01 and 100000.00");
        }
    }
    ...
    private static String fingerprint(OrderRequest request) {
        return String.join("|", request.orderId(), request.sku(), Integer.toString(request.quantity()),
